...
@Component({
  selector: 'out-return-customer-note-add-edit-dialog',
  templateUrl: './out-return-customer-note-add-edit-dialog.component.html',
  styleUrls: ['./out-return-customer-note-add-edit-dialog.component.css'],
})
export class OutReturnCustomerNoteAddEditDialogComponent extends BaseComponent implements OnInit {
  listPaymentType: any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<OutReturnCustomerNoteAddEditDialogComponent>,
    ...
    this.formData = this.fb.group({
      ...
      loaiPhieu: [LOAI_THU_CHI.CHI_TRA_LAI_KHACH_HANG],
      amount: [0],
      dienGiai: [''],
      paymentTypeId: [0],
    ...
    if ($event) {
      ...
      if (this.formData.value?.debtNotes.length >= 2) {
        this.formData.value?.debtNotes.unshift({
          id: 0,
          noteInfo: '--Tất cả--',
          debtAmount: this.formData.value?.debtAmount,
  ...
  async openAddEditInComingCustomerNoteAddEditDialog() {
    this.closeModal();
    const config = {
      data: {
        customerId: this.formData.value?.customerId
