...
import {BankAccountListDialogComponent} from "../bank-account-list-dialog/bank-account-list-dialog.component";
import {
  DrugStorePickerListDialogComponent
} from "../drug-store-picker-list-dialog/drug-store-picker-list-dialog.component";
import {UserPickerListDialogComponent} from "../user-picker-list-dialog/user-picker-list-dialog.component";
...
export class DrugStoreAddEditDialogComponent extends BaseComponent implements OnInit {
  ...
  async openUserPickerDialog() {
    const dialogRef = this.dialog.open(UserPickerListDialogComponent, {
      data: null,
      width: '600px',
    });
    dialogRef.afterClosed().subscribe(async result => {
      if (result) {
        ...
        this.drugStore.nguoiPhuTrach = result.tenDayDu;
      }
    });
  }
  ...
  async openDrugStorePickerDialog() {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result) {
        ...
        console.log(result)
        this.drugStore.nhaThuocQuanLy = result.tenNhaThuoc;
      }
    });
  }
